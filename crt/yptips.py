...
def exe_host():
    path = r'Desktop' + '\\'
    exe_list = ['YpTips.exe ', 'YpTips-mx.exe ', 'xftrayflash.exe ', 'wxhardware.exe ', 'ktelfin.exe ']
    url_host = ["http://test-ssp.7654.com/tray", "http://ssp.7654.com/tray", 'http://test-ssp.7654.com/ys', 'http://domain.thorzip.muxin.fun/ys',
...
def argue():
    ...
    if proj in range(1, 7):
        adurl = '-adurl=%s -pageurl=%s -qid=%s -ad=%s_yptips_1' % (host, host, project, qid_ssp)
    ...
    arg = '%s -taskid=tray-py -killprocess=1 -usewebmode=1 -localcity="吐鲁番" -mutex=%s -project=%s %s -reportold=0 -countdown=10 -waitbrowser=0 -showid=' \
